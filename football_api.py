...
Hizli veri cekme: once genis aralik, takim filtresi
"""
import requests
import datetime
import time
...
# ─────────────────────────────────────────────
# TAKIM SON MACLARI
# Tek istekte genis aralik, takim filtresi
# ─────────────────────────────────────────────
...
def _fetch_matches_in_range(team_id, from_date, to_date):
    """Verilen tarih araliginda takim maclarini getirir. TEK ISTEK."""
    result = _get({
        "met":  "Fixtures",
        "from": from_date,
        "to":   to_date,
...
def _get_team_all_matches(team_id, needed=12):
    """
    Takimin son bitmis lig maclarini ceker.
    Once son 60 gun, yetmezse 120 gun, yetmezse 180 gun.
    """
    today     = datetime.date.today()
    yesterday = today - datetime.timedelta(days=1)

    # Adim adim geriye git
    for days_back in [60, 120, 180]:
        from_date = (yesterday - datetime.timedelta(days=days_back)).strftime("%Y-%m-%d")
        to_date   = yesterday.strftime("%Y-%m-%d")

        matches = _fetch_matches_in_range(team_id, from_date, to_date)

        # Yeterli mac var mi?
        home_matches = [m for m in matches if m.get("is_home")]
        away_matches = [m for m in matches if not m.get("is_home")]

        if len(matches) >= needed or (len(home_matches) >= 6 and len(away_matches) >= 6):
            return matches
...
def get_team_last_matches(team_id, limit=6):
    matches = _get_team_all_matches(team_id, needed=limit)
    return matches[-limit:]


def get_team_home_matches(team_id, limit=6):
    matches = _get_team_all_matches(team_id, needed=limit * 3)
...
def get_match_data(home_id, away_id):
    """
    4 kez _get_team_all_matches cagrisi yapar.
    Her biri max 3 API istegi = toplam max 12 istek.
    """
