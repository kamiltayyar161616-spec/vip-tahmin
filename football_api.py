...
Turkiye saati (UTC+3) duzeltmesi ile
"""
import requests
import datetime
import os
...
def get_fixtures_by_date(date_str):
    """
    Verilen Turkiye tarihine gore UTC araligi hesaplar
    ve tum futbol maclarini dondurur.
    """
    # Turkiye'de o gun = UTC'de bir onceki gun 21:00 - o gun 21:00
    # date_str Turkiye tarihi, biz UTC araligini hesapliyoruz
    try:
        tr_day   = datetime.datetime.strptime(date_str, "%Y-%m-%d")
        utc_from = tr_day - datetime.timedelta(hours=TZ_OFFSET)
        utc_to   = tr_day + datetime.timedelta(hours=24 - TZ_OFFSET)
        from_str = utc_from.strftime("%Y-%m-%d")
        to_str   = utc_to.strftime("%Y-%m-%d")
    except Exception:
        from_str = to_str = date_str

    # Bir veya iki tarih araliginda cek
    if from_str == to_str:
        result = _get({"met": "Fixtures", "from": from_str, "to": to_str}) or []
    else:
        r1 = _get({"met": "Fixtures", "from": from_str, "to": from_str}) or []
        r2 = _get({"met": "Fixtures", "from": to_str,   "to": to_str})   or []
        result = r1 + r2
    ...
    for ev in result:
        try:
            ...
            # Maç saatini UTC'den TR'ye cevir
            raw_time = ev.get("event_time", "00:00")[:5]  # HH:MM
            raw_date = ev.get("event_date", from_str)

            utc_dt = datetime.datetime.strptime(f"{raw_date} {raw_time}", "%Y-%m-%d %H:%M")
            tr_dt  = utc_dt + datetime.timedelta(hours=TZ_OFFSET)
