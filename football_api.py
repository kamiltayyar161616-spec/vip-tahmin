...
def get_fixtures_by_date(date_str):
    """
    Verilen tarihteki fiksturu dondurur.
    date_str: 'YYYY-MM-DD'
    """
    data = _get("fixtures/matches.json", {"date": date_str})
    if not data or "fixtures" not in data:
        # Bos liste donuyor olabilir
        fixtures = data.get("fixtures", []) if data else []
        if not fixtures:
            return []
...
def get_team_last_matches(team_id, limit=6):
    ...
    if not data:
        return []
    matches_raw = data.get("matches", data.get("match", []))
    results = []
