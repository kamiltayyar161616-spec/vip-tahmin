...
def _get_league_matches(league_id):
    """Ligin tum sezon maclarini ceker, sadece gerekli alanlar cache'lenir."""
    ...
    # Sadece gerekli alanlari tut, bellegi bosalt
    slim = []
    for ev in result:
        try:
            slim.append({
...
def get_team_matches_from_league(team_id, league_id, limit=12):
    """Lig maclarindan takim verisi ceker. En yeni maclar once."""
    ...
    for ev in league_matches:
        ...
        if p:
            parsed.append(p)
    # API en yeniden eskiye donduruyor, son limit macu al
    return parsed[:limit]
