...
AllSports API - duzeltilmis versiyon
"""
import requests
import datetime
import os
...
CUP_KW = [
    ...
    'friendlies', 'amical', 'international', 'women', 'u20', 'u21', 'u23'
]
...
def _parse_score(raw):
    """'2 - 1' veya '2-1' formatini parse et"""
    if not raw:
        return None, None
    raw = str(raw).strip()
    for sep in [" - ", "-", ":"]:
...
def get_fixtures_by_date(date_str):
    ...
    for ev in results:
        try:
            ...
            # Saat - AllSports UTC+2 donduruyor, TR icin +1 ekliyoruz
...
def get_team_matches(team_id):
    """
    Takimin son 6 aylik bitirmis maclarini cek.
    AllSports: met=Fixtures & teamId=X & from=...&to=...
    """
