...
concurrent.futures kaldirildi, gunicorn ile uyumlu
"""
import os
import datetime
...
def run_analysis(home_id: int, away_id: int) -> dict:
    """Veri cek, analiz yap, rating ekle."""
    try:
...
def api_analyze(home_id: int, away_id: int):
    cache_key = f"analysis_v3_{home_id}_{away_id}"
    cached    = cache.get(cache_key)
    if cached:
        return jsonify({"success": True, "analysis": cached})
    try:
        result = run_analysis(home_id, away_id)
