...
cache_config = {
    ...
    "CACHE_DEFAULT_TIMEOUT": 3600,  # 1 saat cache
}
...
def compute_rating(matches: list, limit: int = 6) -> int:
...
def analyze_match(match: dict) -> dict:
    cache_key = f"analysis_v2_{match['home_id']}_{match['away_id']}"
    ...
    try:
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            try:
                data = future.result(timeout=60)  # 60 saniye timeout
            except concurrent.futures.TimeoutError:
                print(f"[timeout] {match['home_id']} vs {match['away_id']}")
...
def api_analyze(home_id: int, away_id: int):
    cache_key = f"analysis_v2_{home_id}_{away_id}"
    cached    = cache.get(cache_key)
    if cached:
