...
def api_debug(team_id, league_id):
    """Takim icin kac mac geldigini goster."""
    matches = fapi._get_league_matches(league_id)
    team_matches = [m for m in matches if 
                    int(m.get("home_team_key",0)) == team_id or 
                    int(m.get("away_team_key",0)) == team_id]
    parsed = []
    skipped = []
    for ev in team_matches:
        p = fapi._parse_team_match(ev, team_id)
        if p:
            p["date"]          = ev.get("event_date","")
            p["home_team_key"] = ev.get("home_team_key")
            p["away_team_key"] = ev.get("away_team_key")
    ...
    return jsonify({
        "total_league_matches": len(matches),
